...
public class Figuras : MonoBehaviour
{
    ...
    void generaCuadro(){
        GameObject espadaC = createSword(new Vector3(40.11f,10.5f,1.07f));
        espadaC.transform.rotation*=Quaternion.Euler(-135,-90,0);
        GameObject espadaC1 = createSword(new Vector3(32.65f,9.84f,1.07f));
   ...
   void createWall(Vector3 position, float x, float y, float z, float xScale,float yScale){
    ...
    void Start()
    {
        ...
        createBed();
        GameObject espada1 =createSword(new Vector3(0.28f,10,14.28f));
        espada1.transform.rotation*=Quaternion.Euler(180,0,0);
        createCandle(new Vector3(2.06f,5.9f,22.18f));
        generaCuadro();
        createAirplane(new Vector3(6.48f,0.96f,5.31f));
    }
